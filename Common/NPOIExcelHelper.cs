...
namespace Common
{
    ...
    public class NPOIExcelHelper
    {
        ...
        /// <param name="message">错误消息</param>
        /// <returns>是否成功</returns>
        public static bool DataSetToExcel(DataSet ds, ref MemoryStream stream, out string message)
        {
            #region 声明部分
            ...
            //写入Excel到文件
            WorkbookWriterToMemoryStream(workbook, stream, out fileMessage);
            message += fileMessage;
        ...
        /// <param name="FilePath">路径</param>
        /// <param name="Method">写入方式，创建或追加</param>
        private static void WorkbookWriterToMemoryStream(
            IWorkbook workbook, 
            MemoryStream stream,
            out string message)
        {
            ...
            try
            {
                    workbook.Write(stream);//向打开的这个xls文件中写入数据 
            }
            catch (Exception exp)
            {
                message = exp.Message;
