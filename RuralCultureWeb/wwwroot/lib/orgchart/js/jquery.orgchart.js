...
(function (factory) {
    ...
    OrgChart.prototype = {
        ...
        endLoading: function ($edge, nodeData) {
            ...
            //#region 修改过的代码，用于应用后端设置的字体颜色
            var nodeObj = $node[0];
            for (var p in nodeObj) {
                if (p.toLowerCase().indexOf("jquery") != -1) {
                    var findNode = nodeObj[p].nodeData;
                    if (findNode.fontColor != '')
                        $("#" + findNode.orgId + " .content").css("color", findNode.fontColor);
                    break;
                }
            }
            //#endregion
        ...
        // load new nodes by ajax ajax动态加载节点方法
        loadNodes: function (rel, url, $edge) {
            ...
            $.ajax({ 'url': url, 'dataType': 'json' })
                .done(function (data) {
                    ...
                    data = data.result;
                    that.currentNodeData = data.result;
                    console.log(data);
                    ...
                    if (data.fontColor != '') {
                        $("#" + data.orgId + " .content").css("color", data.fontColor);
                    }
                })
                .fail(function () {
        ...
        topEdgeClickHandler: function (event) {
            ...
            console.log("topEdgeClickHandler");
            console.log(event);
            var that = this;
